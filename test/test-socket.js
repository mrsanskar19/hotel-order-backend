...
socket.on("connect", () => {
  ...
  // 1. Join a hotel
  socket.emit("join_hotel", { hotelId: 1 });
...
socket.on("order_created", (order) => console.log("🆕 Order created:", order));
socket.on("new_order", (order) => console.log("📢 Hotel notified of new order:", order));
socket.on("order_updated", (order) => console.log("➕ Order updated:", order));
socket.on("order_closed", (order) => console.log("✅ Order closed:", order));
