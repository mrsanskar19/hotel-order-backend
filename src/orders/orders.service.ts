...
export class OrderService {
  ...
  async create(
    ...
    if (
      ...
      throw new BadRequestException(
        'Customer ID, total amount, payment mode, and items are required',
      );
    }
    ...
    for (const item of data.items) {
      const menuItem = await this.prisma.menuItem.findUnique({
        where: { item_id: item.item_id, hotel_id: hotelId },
      });
      if (!menuItem || !menuItem.available) {
        throw new BadRequestException(
          `Item ${item.item_id} is invalid or unavailable`,
        );
      }
  ...
  async update(
    ...
    if (
      data.payment_mode &&
      !Object.values(PaymentMethod).includes(data.payment_mode)
    ) {
      throw new BadRequestException('Invalid payment mode');
    }
